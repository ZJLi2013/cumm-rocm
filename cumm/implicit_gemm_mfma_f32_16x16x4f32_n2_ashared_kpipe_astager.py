"""AStager-thinned K-pipe variant for mfma_f32_16x16x4f32_n2_ashared.

This variant keeps the current per-kv BLOCK_K pipeline, but factors sparse A
row lookup out of the per-element A staging loop:
...
MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_ASTAGER_COMPILED_KERNELS: Dict = {}


def _compile_implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_astager(
    ...
    if dtype_str != "f32":
        raise ValueError("mfma_f32_16x16x4f32_n2_ashared_kpipe_astager supports f32 only")
    ...
    allocator = SmemAllocator(
        None,
        arch="gfx942",
        global_sym_name="smem_ig_mfma_f32_16x16x4f32_n2_ashared_kpipe_astager",
    )
...
def implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_astager_forward(
    ...
    """Run the f32 16x16x4 MFMA N2 AStager K-pipe kernel."""
    try:
    ...
    if key not in MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_ASTAGER_COMPILED_KERNELS:
        try:
            ...
            MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_ASTAGER_COMPILED_KERNELS[key] = (
                launch_fn,
                block_m,
            )
            ...
            warnings.warn(
                "Failed to compile mfma_f32_16x16x4f32_n2_ashared_kpipe_astager "
