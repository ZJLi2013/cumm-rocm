...
This version is intentionally narrow:
  - f32 only
  - one wave owns one 16 x 16 output tile
...
It is a fragment-layout experiment, not the final sparse iterator pipeline.
"""
import math
import warnings
...
def _compile_implicit_gemm_v8c(c_in: int, c_out: int, kv: int, dtype_str: str):
    ...
    def kernel(
        ...
        # MFMA 16x16x4f32 lane layout:
        # A lane owns row = lane % 16 and k = lane / 16.
        # B lane owns col = lane % 16 and the same k lane.
        lane = tid
