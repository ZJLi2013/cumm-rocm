...
_V7_COMPILED_KERNELS: Dict = {}


def _compile_implicit_gemm_v7(c_in: int, c_out: int, kv: int, dtype_str: str):
    _ensure_flydsl_path()
    ...
    if dtype_str == 'f32':
        kernel = _make_kernel_v7(is_f32=True)
    elif dtype_str == 'f16':
        kernel = _make_kernel_v7(is_f32=False, use_f16=True)
    else:
        kernel = _make_kernel_v7(is_f32=False, use_f16=False)
...
def implicit_gemm_v7_forward(
    ...
    key = ('v7', c_in, c_out, kv, dtype_str)
    if key not in _V7_COMPILED_KERNELS:
        try:
            launch_fn, block_m = _compile_implicit_gemm_v7(c_in, c_out, kv, dtype_str)
            _V7_COMPILED_KERNELS[key] = (launch_fn, block_m)
        except Exception as e:
            import traceback
            warnings.warn(f"Failed to compile V7 implicit GEMM kernel: {e}\n{traceback.format_exc()}")
            return None
    else:
        launch_fn, block_m = _V7_COMPILED_KERNELS[key]
