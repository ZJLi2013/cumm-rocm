"""mfma_f32_16x16x4f32_n2_ashared implicit GEMM family member.

This variant keeps the n2 work ownership but stages A into block-shared LDS:
  - f32 only
  - one block owns one 16 x 32 output tile
  - two waves/block, each wave owns one 16 x 16 N tile
  - A/features: global/LUT gather -> shared LDS -> VGPR scalar fragment
  - B/weight: global -> LDS -> VGPR scalar fragment per wave
  - C: VGPR accumulator fragment updated by mfma_f32_16x16x4f32
"""
import math
import warnings
...
MFMA_F32_16X16X4F32_N2_ASHARED_COMPILED_KERNELS: Dict = {}


def _compile_implicit_gemm_mfma_f32_16x16x4f32_n2_ashared(
    ...
    if dtype_str != "f32":
        raise ValueError("mfma_f32_16x16x4f32_n2_ashared currently supports f32 only")
    ...
    A_TILE_BYTES = A_TILE_ELEMS * 4
    W_TILE_ELEMS = C_IN * C_OUT_TILE
    W_TILE_ELEMS_PER_BLOCK = W_TILE_ELEMS * WAVES_PER_BLOCK
    W_TILE_BYTES = W_TILE_ELEMS_PER_BLOCK * 4
    allocator = SmemAllocator(
        None, arch="gfx942", global_sym_name="smem_ig_mfma_f32_16x16x4f32_n2_ashared"
    )
    ...
    A_LOAD_PER_BLOCK = math.ceil(A_TILE_ELEMS / BLOCK_THREADS)
    LDG_VEC = min(4, W_TILE_ELEMS)
    W_VEC_LOAD_PER_WAVE = math.ceil(W_TILE_ELEMS / (64 * LDG_VEC))
    ...
    def kernel(
        ...
        smem_a_ptr = SmemPtr(base_ptr, smem_a_offset, T.f32, shape=(A_TILE_ELEMS,))
        a_lds = STensor(smem_a_ptr, T.f32, shape=(A_TILE_ELEMS,))
        smem_w_ptr = SmemPtr(
            base_ptr, smem_w_offset, T.f32, shape=(W_TILE_ELEMS_PER_BLOCK,)
        )
        w_lds = STensor(smem_w_ptr, T.f32, shape=(W_TILE_ELEMS_PER_BLOCK,))
        ...
        # MFMA 16x16x4f32 lane layout within each wave.
        ...
        wave_w_offset = fx.Index(wave_id) * fx.Index(const_expr(W_TILE_ELEMS))
        ...
        for k in range_constexpr(KV):
            ...
            with ir.InsertionPoint(kv_if.then_block):
                # Stage shared A tile once per block for both N-tile waves.
                for ai in range_constexpr(A_LOAD_PER_BLOCK):
                    a_elem_idx = (
                        fx.Index(const_expr(ai * BLOCK_THREADS))
                        + fx.Index(tid)
                    )
                    a_valid = arith.cmpi(
                        arith.CmpIPredicate.ult,
                        a_elem_idx,
                        fx.Index(const_expr(A_TILE_ELEMS)),
                    )
                    a_if = scf.IfOp(a_valid, results_=[], has_else=False)
                    with ir.InsertionPoint(a_if.then_block):
                        a_row_idx = a_elem_idx // fx.Index(const_expr(C_IN))
                        a_c_idx = a_elem_idx % fx.Index(const_expr(C_IN))
                ...
                # Stage B independently per wave for its N tile.
                w_base = fx.Index(
                ...
                for wi in range_constexpr(W_VEC_LOAD_PER_WAVE):
                    w_elem_idx = (
                        fx.Index(const_expr(wi * 64 * LDG_VEC))
                        + fx.Index(lane) * fx.Index(const_expr(LDG_VEC))
                    )
                    w_valid = arith.cmpi(
                        arith.CmpIPredicate.ult,
                        w_elem_idx,
                    ...
                    with ir.InsertionPoint(w_if.then_block):
                        ...
                        w_lds.vec_store(
                            (wave_w_offset + w_elem_idx,), w_vec, const_expr(LDG_VEC)
                        )
                        scf.YieldOp([])
                gpu.barrier()

                for c0 in range_constexpr(0, C_IN, 4):
                    a_off = (
                        fx.Index(mfma_row) * fx.Index(const_expr(C_IN))
                        + fx.Index(const_expr(c0))
...
def implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_forward(
    ...
    """Run the f32 16x16x4 MFMA N2 A-shared implicit GEMM kernel."""
    try:
    ...
    key = ("mfma_f32_16x16x4f32_n2_ashared", c_in, c_out, kv, dtype_str)
    if key not in MFMA_F32_16X16X4F32_N2_ASHARED_COMPILED_KERNELS:
        try:
            launch_fn, block_m = _compile_implicit_gemm_mfma_f32_16x16x4f32_n2_ashared(
                c_in, c_out, kv, dtype_str
            )
            MFMA_F32_16X16X4F32_N2_ASHARED_COMPILED_KERNELS[key] = (launch_fn, block_m)
        except Exception as e:
            import traceback

            warnings.warn(
                "Failed to compile mfma_f32_16x16x4f32_n2_ashared implicit GEMM "
                f"kernel: {e}\n{traceback.format_exc()}"
            )
            return None
    else:
        launch_fn, block_m = MFMA_F32_16X16X4F32_N2_ASHARED_COMPILED_KERNELS[key]
