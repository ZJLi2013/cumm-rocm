...
def _compile_implicit_gemm_v8(c_in: int, c_out: int, kv: int, dtype_str: str):
    ...
    if dtype_str == "f32":
        kernel = _make_kernel_v8(is_f32=True)
    elif dtype_str == "f16":
        kernel = _make_kernel_v8(is_f32=False, use_f16=True)
    else:
        kernel = _make_kernel_v8(is_f32=False, use_f16=False)
...
def implicit_gemm_v8_forward(
    ...
    if key not in _V8_COMPILED_KERNELS:
        try:
            ...
            warnings.warn(
                f"Failed to compile V8 implicit GEMM kernel: {e}\n{traceback.format_exc()}"
            )
            return None
    else:
