"""mfma_f32_16x16x4f32_n2 implicit GEMM family member.

This is the first block-level expansion of the minimal MFMA kernel:
  - f32 only
...
MFMA_F32_16X16X4F32_N2_COMPILED_KERNELS: Dict = {}


def _compile_implicit_gemm_mfma_f32_16x16x4f32_n2(
    ...
    if dtype_str != "f32":
        raise ValueError("mfma_f32_16x16x4f32_n2 currently supports f32 only")
    ...
    allocator = SmemAllocator(
        None, arch="gfx942", global_sym_name="smem_ig_mfma_f32_16x16x4f32_n2"
    )
...
def implicit_gemm_mfma_f32_16x16x4f32_n2_forward(
    ...
    """Run the f32 16x16x4 MFMA N2 implicit GEMM kernel."""
    try:
    ...
    key = ("mfma_f32_16x16x4f32_n2", c_in, c_out, kv, dtype_str)
    if key not in MFMA_F32_16X16X4F32_N2_COMPILED_KERNELS:
        try:
            launch_fn, block_m = _compile_implicit_gemm_mfma_f32_16x16x4f32_n2(
                c_in, c_out, kv, dtype_str
            )
            MFMA_F32_16X16X4F32_N2_COMPILED_KERNELS[key] = (launch_fn, block_m)
        except Exception as e:
            import traceback

            warnings.warn(
                "Failed to compile mfma_f32_16x16x4f32_n2 implicit GEMM "
                f"kernel: {e}\n{traceback.format_exc()}"
            )
            return None
    else:
        launch_fn, block_m = MFMA_F32_16X16X4F32_N2_COMPILED_KERNELS[key]
