...
_V4_COMPILED_KERNELS: Dict = {}


def _compile_implicit_gemm_v4(c_in: int, c_out: int, kv: int, dtype_str: str):
    """V4 implicit GEMM: lut-based gather + register blocking dot product."""
    import sys
    _flydsl_root = os.path.dirname(os.path.dirname(__import__('flydsl').__file__))
    if _flydsl_root not in sys.path:
        sys.path.insert(0, _flydsl_root)
    ...
    allocator = SmemAllocator(None, arch="gfx942", global_sym_name="smem_ig_v4")
    smem_w_offset = allocator._align(allocator.ptr, 16)
    W_BYTES = W_ELEMS * DT_BYTES
    allocator.ptr = smem_w_offset + W_BYTES
    ...
    def _make_kernel_v4(is_f32=True, use_f16=True):
        NEED_EXTF = not is_f32
        DT_TAG = 'f32' if is_f32 else ('f16' if use_f16 else 'bf16')
    ...
    if dtype_str == 'f32':
        kernel = _make_kernel_v4(is_f32=True)
    elif dtype_str == 'f16':
        kernel = _make_kernel_v4(is_f32=False, use_f16=True)
    else:
        kernel = _make_kernel_v4(is_f32=False, use_f16=False)
...
def implicit_gemm_v4_forward(
    ...
    """V4: lut-based gather + register blocking dot product."""
    try:
        import flydsl
    except ImportError:
        return None
    ...
    key = ('v4', c_in, c_out, kv, dtype_str)
    if key not in _V4_COMPILED_KERNELS:
        try:
            launch_fn, block_m = _compile_implicit_gemm_v4(c_in, c_out, kv, dtype_str)
            _V4_COMPILED_KERNELS[key] = (launch_fn, block_m)
        except Exception as e:
            import traceback
            warnings.warn(f"Failed to compile V4 implicit GEMM kernel: {e}\n{traceback.format_exc()}")
            return None
    else:
        launch_fn, block_m = _V4_COMPILED_KERNELS[key]
...
# ---------------------------------------------------------------------------
# V5: K-fused implicit GEMM — K = C_IN × active_kv
# ---------------------------------------------------------------------------
...
def implicit_gemm_v5_forward(
    ...
    key = ('v5', c_in, c_out, kv, dtype_str)
    if key not in _V5_COMPILED_KERNELS:
        try:
            launch_fn, block_m = _compile_implicit_gemm_v5(c_in, c_out, kv, dtype_str)
            _V5_COMPILED_KERNELS[key] = (launch_fn, block_m)
        except Exception as e:
            import traceback
            warnings.warn(f"Failed to compile V5 implicit GEMM kernel: {e}\n{traceback.format_exc()}")
            return None
    else:
        launch_fn, block_m = _V5_COMPILED_KERNELS[key]
