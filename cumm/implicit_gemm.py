...
def _compile_implicit_gemm_v4(c_in: int, c_out: int, kv: int, dtype_str: str):
    ...
    Eliminates pair scan loop — each thread reads its inp_row from a
    precomputed lookup table, gathers features to registers, then does
    vectorized dot product with weight in LDS.
    """
    import sys
    _flydsl_root = os.path.dirname(os.path.dirname(__import__('flydsl').__file__))
    if _flydsl_root not in sys.path:
