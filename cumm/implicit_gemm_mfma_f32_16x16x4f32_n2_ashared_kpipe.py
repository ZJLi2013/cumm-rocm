...
This is a lightweight KStager prototype:
  - f32 only
  - one block owns one 16 x 32 output tile
  - two waves/block, each wave owns one 16 x 16 N tile
  - A/features: global/LUT gather -> shared LDS [16 x BLOCK_K]
  - B/weight: global -> LDS [BLOCK_K x 16] per wave
  - C: VGPR accumulator fragment updated by mfma_f32_16x16x4f32

It keeps the current LUT representation and stages K in BLOCK_K chunks inside
each kv position. It is intentionally not a full MaskIGemmIterator yet.
"""
import math
import warnings
...
def _compile_implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe(
    ...
    def kernel(
        ...
        smem_a_ptr = SmemPtr(base_ptr, smem_a_offset, T.f32, shape=(A_STAGE_ELEMS,))
        a_lds = STensor(smem_a_ptr, T.f32, shape=(A_STAGE_ELEMS,))
        smem_w_ptr = SmemPtr(
            base_ptr, smem_w_offset, T.f32, shape=(W_STAGE_ELEMS_PER_BLOCK,)
        )
        ...
        for k in range_constexpr(KV):
            ...
            with ir.InsertionPoint(kv_if.then_block):
                for c_block in range_constexpr(0, C_IN, BLOCK_K):
                    # Stage A[16 x BLOCK_K], padding channels outside C_IN with zero.
                    ...
                    # Stage B[BLOCK_K x 16] independently per wave.
                    w_base = fx.Index(
...
def implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_forward(
    ...
    """Run the f32 16x16x4 MFMA N2 A-shared KStager prototype."""
    try:
    ...
    if key not in MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_COMPILED_KERNELS:
        try:
            ...
            MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_COMPILED_KERNELS[key] = (launch_fn, block_m)
        except Exception as e:
            import traceback

            warnings.warn(
                "Failed to compile mfma_f32_16x16x4f32_n2_ashared_kpipe implicit GEMM "
