"""Double-buffered K-pipe variants for mfma_f32_16x16x4f32_n2_ashared.

This keeps the lightweight LUT-based KStager shape, but uses two LDS stages:
  - preload current K block into stage 0
  - while computing current stage, pre-stage the next K block into stage 1
  - barrier and swap stages

It is a software ping-pong prototype, not a full async copy pipeline.
"""
import math
import warnings
...
MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_DB_COMPILED_KERNELS: Dict = {}


def _compile_implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db(
    ...
    if dtype_str != "f32":
        raise ValueError("mfma_f32_16x16x4f32_n2_ashared_kpipe_db supports f32 only")
    if block_k not in (32, 64):
        raise ValueError(f"unsupported block_k={block_k}, expected 32 or 64")
    ...
    A_TOTAL_ELEMS = A_STAGE_ELEMS * LDS_STAGES
    W_TOTAL_ELEMS = W_STAGE_ELEMS_PER_BLOCK * LDS_STAGES
    ...
    allocator.ptr = smem_w_offset + W_STAGE_BYTES * LDS_STAGES
    ...
    def kernel(
        ...
        w_lds = STensor(smem_w_ptr, T.f32, shape=(W_TOTAL_ELEMS,))
        ...
        for k in range_constexpr(KV):
            ...
            with ir.InsertionPoint(kv_if.then_block):
                ...
                for c_block in range_constexpr(0, C_IN, BLOCK_K):
                    ...
                    read_stage = (c_block // BLOCK_K) % 2
                    write_stage = read_stage ^ 1
                    next_c_block = c_block + BLOCK_K

                    if const_expr(next_c_block < C_IN):
                        # Pre-stage the next K block into the alternate LDS stage.
                        for ai in range_constexpr(A_LOAD_PER_BLOCK):
                            a_elem_idx = fx.Index(const_expr(ai * BLOCK_THREADS)) + fx.Index(tid)
                            a_valid = arith.cmpi(
                                arith.CmpIPredicate.ult,
                            ...
                            with ir.InsertionPoint(a_if.then_block):
                                ...
                                a_val = arith.select(load_a, a_val, zero_f32)
                                a_stage_off = (
                                    fx.Index(const_expr(write_stage * A_STAGE_ELEMS)) + a_elem_idx
                                )
                                a_lds.store(a_stage_off, a_val)
                                scf.YieldOp([])

                        w_base_next = fx.Index(
                            const_expr(k * N_C_OUT_TILES * C_IN * C_OUT_TILE)
                        ) + fx.Index(ct) * fx.Index(const_expr(C_IN * C_OUT_TILE))
                        for wi in range_constexpr(W_LOAD_PER_WAVE):
                            w_elem_idx = fx.Index(const_expr(wi * 64)) + fx.Index(lane)
                            ...
                            with ir.InsertionPoint(w_if.then_block):
                                ...
                                w_col_idx = w_elem_idx % fx.Index(const_expr(C_OUT_TILE))
                                w_k_i32 = fx.Int32(arith.index_cast(T.i32, w_k_idx))
                                c_idx_i32 = w_k_i32 + fx.Int32(const_expr(next_c_block))
                                c_valid = arith.cmpi(
                                    arith.CmpIPredicate.slt,
                                    c_idx_i32,
                                    fx.Int32(const_expr(C_IN)),
                                ...
                                b_val = arith.select(c_valid, b_val, zero_f32)
                                w_stage_off = (
                                    fx.Index(const_expr(write_stage * W_STAGE_ELEMS_PER_BLOCK))
                                    + wave_w_offset
                                    + w_elem_idx
                                )
                                w_lds.store(w_stage_off, b_val)
                                scf.YieldOp([])

                    for kk in range_constexpr(0, BLOCK_K, 4):
                        a_off = (
                            fx.Index(const_expr(read_stage * A_STAGE_ELEMS))
                            + fx.Index(mfma_row) * fx.Index(const_expr(BLOCK_K))
                            + fx.Index(const_expr(kk))
                            + fx.Index(mfma_k_lane)
                        )
                        a_val = a_lds.load(a_off)
                        b_off = (
                            fx.Index(const_expr(read_stage * W_STAGE_ELEMS_PER_BLOCK))
                            + wave_w_offset
...
def _implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db_forward(
    ...
    key = (
        f"mfma_f32_16x16x4f32_n2_ashared_kpipe_db{block_k}",
        c_in,
        c_out,
        kv,
        dtype_str,
    ...
    if key not in MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_DB_COMPILED_KERNELS:
        try:
            launch_fn, block_m = _compile_implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db(
                c_in, c_out, kv, dtype_str, block_k
            )
            MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_DB_COMPILED_KERNELS[key] = (
                launch_fn,
                block_m,
            )
            ...
            warnings.warn(
                "Failed to compile mfma_f32_16x16x4f32_n2_ashared_kpipe_db "
                f"kernel: {e}\n{traceback.format_exc()}"
            )
            return None
    else:
        launch_fn, block_m = MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_DB_COMPILED_KERNELS[key]
...
def implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db32_forward(
    features: torch.Tensor,
    filters: torch.Tensor,
    indice_pairs: torch.Tensor,
    indice_pair_num: torch.Tensor,
    ...
    """Run the f32 16x16x4 MFMA N2 A-shared K-pipe DB32 kernel."""
    return _implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db_forward(
        features, filters, indice_pairs, indice_pair_num, num_activate_out, 32
    )


def implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db64_forward(
    features: torch.Tensor,
    filters: torch.Tensor,
    indice_pairs: torch.Tensor,
    indice_pair_num: torch.Tensor,
    ...
    """Run the f32 16x16x4 MFMA N2 A-shared K-pipe DB64 kernel."""
    return _implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db_forward(
        features, filters, indice_pairs, indice_pair_num, num_activate_out, 64
    )
