...
echo "=== Counters: LDS padding ==="
FLYDSL_RUNTIME_ENABLE_CACHE=0 WORKLOAD_DIR=/tmp/kpipe_step5/workloads_padding VARIANTS=direct SHAPES=64x128 ITERS=20 WARMUP=5 INSTALL_ROCPROF_COMPUTE_DEPS=1 \
  bash /tmp/cumm-rocm/profile/run_rocprof_compute_kpipe.sh 2>&1 | tail -20

echo "=== Extract counters (padding vs A-vec4 baseline) ==="
python3 /tmp/cumm-rocm/profile/extract_counters.py \
  /tmp/kpipe_step5/workloads_padding \
  /tmp/kpipe_step3/workloads_vec4 \
