...
def bench_config(n_active, c_in, c_out, kv, density, dtype, device):
    ...
    impl_mfma_n2_ashared_kpipe = (
        implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_forward(
            features, filters, ip, ipn, n_active
        )
    )
    if impl_mfma_n2_ashared_kpipe is not None:
        print(
            "  mfma_f32_16x16x4f32_n2_ashared_kpipe max error: "
            f"{(impl_mfma_n2_ashared_kpipe.float() - ref.float()).abs().max().item():.6f}"
        )
        bench_fn(
            lambda: implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_forward(
                features, filters, ip, ipn, n_active
            ),
            label="[K3] mfma_f32_16x16x4f32_n2_ashared_kpipe full",
        )
        mfma_n2_ashared_kpipe_key = (
            "mfma_f32_16x16x4f32_n2_ashared_kpipe",
            c_in,
            c_out,
            kv,
        ...
        if (
            mfma_n2_ashared_kpipe_key
            in MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_COMPILED_KERNELS
            and hip is not None
        ):
            launch_fn, block_m = MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_COMPILED_KERNELS[
                mfma_n2_ashared_kpipe_key
            ]
            num_tiles = (n_active + block_m - 1) // block_m
            _, _, _, mask, _, _, lut = hip.build_implicit_gemm_mask(ip, ipn, n_active, block_m)
            ...
            bench_fn(
                ...
                label="[K3.3] mfma_f32_16x16x4f32_n2_ashared_kpipe kernel only",
            )
    else:
        print("  [WARN] mfma_f32_16x16x4f32_n2_ashared_kpipe failed, skipping")

    astager_name = "mfma_f32_16x16x4f32_n2_ashared_kpipe_astager"
    impl_mfma_n2_ashared_kpipe_astager = (
        implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_astager_forward(
            features, filters, ip, ipn, n_active
        )
    )
    if impl_mfma_n2_ashared_kpipe_astager is not None:
        print(
            f"  {astager_name} max error: "
            f"{(impl_mfma_n2_ashared_kpipe_astager.float() - ref.float()).abs().max().item():.6f}"
        )
        bench_fn(
            lambda: implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_astager_forward(
                features, filters, ip, ipn, n_active
            ),
            label=f"[K3-a] {astager_name} full",
        )
        astager_key = (astager_name, c_in, c_out, kv, dtype_str)
        if (
            astager_key in MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_ASTAGER_COMPILED_KERNELS
            and hip is not None
        ):
            launch_fn, block_m = (
                MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_ASTAGER_COMPILED_KERNELS[
            ...
            def mfma_n2_ashared_kpipe_astager_kernel_only():
            ...
            bench_fn(
                mfma_n2_ashared_kpipe_astager_kernel_only,
                label=f"[K3-a.3] {astager_name} kernel only",
            )
    else:
        print(f"  [WARN] {astager_name} failed, skipping")

    for block_k, db_forward in (
        (32, implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db32_forward),
        (64, implicit_gemm_mfma_f32_16x16x4f32_n2_ashared_kpipe_db64_forward),
    ):
        db_name = f"mfma_f32_16x16x4f32_n2_ashared_kpipe_db{block_k}"
        impl_db = db_forward(features, filters, ip, ipn, n_active)
        if impl_db is not None:
            print(
                f"  {db_name} max error: "
                f"{(impl_db.float() - ref.float()).abs().max().item():.6f}"
            )
            bench_fn(
                lambda fn=db_forward: fn(features, filters, ip, ipn, n_active),
                label=f"[K3-db{block_k}] {db_name} full",
            )
            db_key = (db_name, c_in, c_out, kv, dtype_str, block_k)
            if (
                db_key in MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_DB_COMPILED_KERNELS
                and hip is not None
            ):
                launch_fn, block_m = (
                    MFMA_F32_16X16X4F32_N2_ASHARED_KPIPE_DB_COMPILED_KERNELS[db_key]
                )
                num_tiles = (n_active + block_m - 1) // block_m
                _, _, _, mask, _, _, lut = hip.build_implicit_gemm_mask(
                    ip, ipn, n_active, block_m
                ...
                def mfma_n2_ashared_kpipe_db_kernel_only():
                    out_features = torch.zeros(
                        n_active, c_out, dtype=torch.float32, device=device
                    )
                    launch_fn(
                ...
                bench_fn(
                    mfma_n2_ashared_kpipe_db_kernel_only,
                    label=f"[K3-db{block_k}.3] {db_name} kernel only",
                )
        else:
            print(f"  [WARN] {db_name} failed, skipping")
