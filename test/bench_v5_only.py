"""Focused benchmark: V4 vs V5 K-fused implicit GEMM (skip V3).

Avoids V3 JIT compilation overhead to keep total runtime manageable.
"""
import time
import torch
import numpy as np
...
def bench_config(n_active, c_in, c_out, kv, density, dtype, device):
    from cumm.implicit_gemm import implicit_gemm_v4_forward, implicit_gemm_v5_forward, _get_hip_module
    ...
    if impl_v4 is not None:
        ...
        from cumm.implicit_gemm import _V4_COMPILED_KERNELS
    ...
    if impl_v5 is not None:
        ...
        bench_fn(lambda: implicit_gemm_v5_forward(features, filters, ip, ipn, n_active),
                 label="[E] Implicit GEMM V5 K-fused (full)")
        ...
        if v5_key in _V5_COMPILED_KERNELS and hip is not None:
            ...
            bench_fn(v5_kernel_only, label="[E.3] V5 Kernel only (K-fused)")
    else:
        print(f"  [WARN] V5 failed, skipping")
