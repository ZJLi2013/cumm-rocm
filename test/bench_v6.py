"""Benchmark: V4 vs V5 vs V6 implicit GEMM (skip V3)."""
import time
import torch
import numpy as np
...
def bench_config(n_active, c_in, c_out, kv, density, dtype, device):
    from cumm.implicit_gemm import (
        implicit_gemm_v4_forward, implicit_gemm_v5_forward, implicit_gemm_v6_forward,
        _get_hip_module, _V4_COMPILED_KERNELS, _V6_COMPILED_KERNELS,
    )
    ...
    if impl_v6 is not None:
        ...
        bench_fn(lambda: implicit_gemm_v6_forward(features, filters, ip, ipn, n_active),
                 label="[G] Implicit GEMM V6 output-tiled (full)")
        ...
        if v6_key in _V6_COMPILED_KERNELS and hip is not None:
            ...
            bench_fn(v6_kernel_only, label="[G.3] V6 Kernel only (output-tiled)")
    else:
        print(f"  [WARN] V6 failed, skipping")
